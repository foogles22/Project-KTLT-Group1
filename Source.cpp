...
void viewCourse_Student(Node_student* student_login, Node_year* current_year)
{
	...
	Node_course* ph_enrolled_coure = student_login->ph_course_enrolled1;
	if (ph_enrolled_coure != NULL && current_year->semestercount == 1)
	{
		viewListofcourse(ph_enrolled_coure);
		print_double_square(120, 14, 12, 3, 15, "Semester 1");
		return;
	}
	ph_enrolled_coure = student_login->ph_course_enrolled2;
	if (ph_enrolled_coure != NULL && current_year->semestercount == 2)
	{
		viewListofcourse(ph_enrolled_coure);
		print_double_square(120, 14, 12, 3, 15, "Semester 2");
		return;
	}
	ph_enrolled_coure = student_login->ph_course_enrolled3;
	if (ph_enrolled_coure != NULL && current_year->semestercount == 3)
	{
		viewListofcourse(ph_enrolled_coure);
		print_double_square(120, 14, 12, 3, 15, "Semester 3");
		return;
	}
...
void add_1st_student(Node_student*& ph_student, string class_name, string year_name)
{
	...
	if (pc_student != NULL)
	{
		print_double_square(30, 15, 32, 3, 15, "Student have been added to this class");
		return;
	}
	while (!read.eof())
	{
...
void enroll_course(Node_student* student_login, Node_year*& current_year)
{
	...
	{
		print_double_square(120, 14, 12, 3, 15, "Semester " + to_string(current_year->semestercount));
...
void deleteEnrolledCourse(Node_student*& student, Node_year*& current_year) {
	...
	if (course_choose != NULL)
	{
		...
		while (course_choose_del->data.id != course_choose->data.id)
