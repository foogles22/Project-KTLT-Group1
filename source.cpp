...
void viewAllStudentInAClass(Node_student* ph_student) {
	...
	print_double_square(76, 40, 104, i - 1, 15, "\0");
	return;
}
void viewListofstudentsincourse(Node_student* ph_student)
{
	...
	print_double_square(76, 30, 104, i - 1, 15, "\0");
	return;
}
//View profile
...
//Hàm check input add các node
int count_year(Node_year* ph_year)
{
	int count = 0;
	while (ph_year != NULL)
	{
...
int count_class(Node_class* ph_class)
{
	int count = 0;
	while (ph_class != NULL)
	{
...
int count_course(Node_course* ph_course)
{
	int count = 0;
	while (ph_course != NULL)
	{
...
void saveStudentData(Node_year* ph_year)
{
	ofstream time("StudentData\\Save\\time.txt");
	time << now;
	time.close();
	ofstream fyear("StudentData\\Save\\CreatedYear.txt");
	while (ph_year != NULL)
	{
		fyear << ph_year->name;
		...
		if (ph_year->ph_semester != NULL)
		{
			ofstream semester_created("StudentData\\" + ph_year->name.substr(2, 2) + "\\Save\\SemesterCreated");
			semester_created.close();
			Node_semester* pc_semester = ph_year->ph_semester;
			while (pc_semester != nullptr)
			{
				if (pc_semester->ph_course != NULL)
					ofstream fcourse("StudentData\\" + ph_year->name.substr(2, 2) + "\\Save\\" + pc_semester->semester_no + "_courseadded.txt");
				pc_semester = pc_semester->semester_next;
			}
		}
...
void ingiaodien_creating_year(Node_year* current_year, Node_year* ph_year, int choose)
{
...
void ingiaodien_after_creating_year(Node_year* year_choose, int choose)
{
	print_duongthang(25, 1, 62);
	print_duongngang(2, 5, 23);
	print_duongngang(26, 5, 208);
...
void ingiaodien_setting(Node_year* current_year, Node_year* ph_year, int choose)
{
	print_duongthang(25, 1, 62);
	print_duongngang(2, 5, 23);
	print_duongngang(26, 5, 208);
...
void print_teacher_setting(Node_year*& ph_year, Node_year*& current_year)
{
	system("CLS");
	print_double_square(30, 10, 33, 3, 15, "Loading your profile completed!");
	ingiaodien_setting(current_year,ph_year, 0);
	_getch();
	int choose = 1;	char move;
	do {
		system("CLS");
		...
		do {
			...
			ingiaodien_setting(current_year, ph_year, choose);
			move = _getch();
			if (move == 'w' || move == 72)
				if (choose > 1)
					choose--;
...
int login(string a, string s1) {
	if (a[0] == '1' || a[0] == '2' || a[0] == '3' || a[0] == '4' || a[0] == '5' || a[0] == '6' || a[0] == '7' || a[0] == '8' || a[0] == '9' || a[0] == '0') {
		...
		while (!fs.eof()) {
			...
			getline(fs, s, ',');
			if (s == a) {
				teacher_login.mail = a;
				getline(fs,teacher_login.fullname,',');
				getline(fs, teacher_login.date, ',');
				getline(fs, teacher_login.cccd, ',');
				getline(fs, teacher_login.pass);
				if (s1 == teacher_login.pass) {
					return 2;
				}
			}
		}
	...
	do {
		...
		if (choose == 3)
		{
			...
			if (mssv != "-1" && password != "\0" && login(mssv, password) == 2)
			{
				print_login_status(47, 15, 1);
				_getch();
				print_teacher_setting(ph_year, current_year);
				system("CLS"); choose = 1;
			}
			else
			{
